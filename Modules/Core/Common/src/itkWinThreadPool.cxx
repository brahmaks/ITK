		...
		try
		{
			...
			for( int i = 0; i < MaxThreads; i++ )
			{
				...
				ThreadHandles[i] = CreateThread( 
					...
					(LPTHREAD_START_ROUTINE)ThreadPool::ThreadExecute,       // thread function name
	...
	*/
	void ThreadPool::DestroyPool(int pollTime = 2)
	{
		...
		for( int i = 0; i < MaxThreads; i++ )
		{
			...
			THREAD_DIAGNOSTIC_PRINT( "Thread with thread handle "<<ThreadHandles[i]<<" exited with exit code : " << dwExit << std::endl);
	...
	int ThreadPool::AssignWork(WinJob winJob)
	{
		try
		{
			...
			case WAIT_OBJECT_0: 
				...
				THREAD_DIAGNOSTIC_PRINT( "Assigning Worker[" << (winJob).Id << "] Address:[" << &winJob << "] to Queue " << std::endl);
	...
	void ThreadPool::RemoveActiveId(int id)
	{
		try
		{
			...
			case WAIT_OBJECT_0: 
				...
				{
					THREAD_DIAGNOSTIC_PRINT( std::endl << "Error occured, couldnt find id in WorkerQueue to mark executed. Id is : " << id
						<< std::endl);
